...
export async function listDocuments(): Promise<AdminDocument[]> {
  throw new Error("Not implemented: finviet-be has no knowledge-base document list endpoint yet");
}

export async function uploadDocument(_input: DocumentUploadInput): Promise<AdminDocument> {
  throw new Error("Not implemented: document upload wiring pending JWT propagation");
}
