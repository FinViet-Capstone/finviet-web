...
export async function listUsers(_params: ListUsersParams): Promise<UsersListResult> {
  throw new HttpError(501, "Not implemented: finviet-be has no admin user-list endpoint yet");
}

export async function setUserActive(_id: string, _isActive: boolean): Promise<AdminCustomerSummary> {
  throw new HttpError(501, "Not implemented: finviet-be has no admin user-lock endpoint yet");
}

export async function triggerPasswordReset(_id: string): Promise<{ sent: boolean }> {
  throw new HttpError(501, "Not implemented: admin-initiated password reset endpoint not wired yet");
}
